...
\imgleft[0.3]{images/image-2026-01-04-09-26-27.png}{
    ...
    \concepttable{
        ...
        \conrow{\red{时钟重叠}}{在实际电路中，反相生成的时钟信号可能会有延迟，导致在某一瞬间，CLK 和 $\overline{\text{CLK}}$ 同时为有效电平。}
    }
}
...
\entry{电路拓扑结构}{由两个完全相同的多路开关锁存器串联构成：左为 Master，右为 Slave。总体为典型的 Master–Slave 锁存器架构。}
\concepttable{
    \conrow{Master（左半部分）}{由传输门 $T_1$（输入开关）与 $T_2$（反馈开关）以及反相器 $I_1, I_2, I_3$ 组成。$I_3$ 的输出记为中间节点 $Q_M$。}
    \conrow{Slave（右半部分）}{由传输门 $T_3$（输入开关）与 $T_4$（反馈开关）以及反相器 $I_4, I_5, I_6$ 组成。$I_6$ 的输出为最终输出 $Q$。}
    \conrow{信号流向}{在写入（Master 透明）阶段，$T_1$ 导通，将输入 $D$ 传至 $I_3$，产生 $Q_M$；在保持（Master 关闭）阶段，$Q_M$ 由 $I_2$ 保持；随后在采样（Slave 透明）阶段，$T_3$ 导通，将 $Q_M$ 传至 $I_6$，产生并保持输出 $Q$。}
    \conrow{要点}{两个半部分时钟互补工作：Master 与 Slave 的输入/反馈传输门互为补相以保证边沿触发行为；$Q_M$ 为内部节点，不直接对外驱动，$Q$ 为对外受控输出。}
}
...
\entry{作业 5-3}{
    已知一个数字电路最坏情况下绝对时钟抖动 $t_{jitter} = 0.05\,\mathrm{ns}$，假设布线后可能的最大时钟偏差绝对值 $t_{skew} = 0.3\,\mathrm{ns}$，要求最高时钟频率大于等于 $400\,\mathrm{MHz}$，其中一级流水线电路逻辑部分的最长延时 $t_{logic} = 1\,\mathrm{ns}$，污染延时 $t_{logicd} = 0.1\,\mathrm{ns}$。两端寄存器的建立时间 $setup = 0.5\,\mathrm{ns}$，维持时间 $hold = 0\,\mathrm{ns}$，传播延时 $t_{clk-q} = 0.4\,\mathrm{ns}$，污染延时 $t_{clk,cd} = 0.2\,\mathrm{ns}$。请回答下列问题：
    \vspace{-5pt}
    \begin{enumerate}[label=(\arabic*)]
        \item 定量说明考虑最坏情况，这段电路是否能够保证满足性能要求？
        \item 考虑最坏情况，这段电路会不会出现竞争（即 hold 错误）？
    \end{enumerate}
}
...
\green{这部分需要我首先复习一下动态逻辑电路的内容（2026-01-03 night）}
