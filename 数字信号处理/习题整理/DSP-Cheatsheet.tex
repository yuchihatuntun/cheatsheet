...
\textbf{解题思路：}直接利用 DFT 定义式 $X(k)=\sum_{n=0}^{N-1}x(n)W_N^{kn}$ 逐项计算，并结合几何级数求和公式、DFT 频移性质及欧拉公式进行化简。

(1) 常数序列 $x(n)=1$：
\[
X(k)=\sum_{n=0}^{N-1}W_N^{kn}
     =\sum_{n=0}^{N-1}e^{-j\frac{2\pi}{N}kn}
...
(2) 单位脉冲序列 $x(n)=\delta(n)$：
\[
X(k)=\sum_{n=0}^{N-1}\delta(n)W_N^{kn}=1,
